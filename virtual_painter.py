...
pdfname='test.pdf'
images = convert_from_path(pdfname, 500)#,poppler_path=r'C:\Program Files\poppler-0.68.0\bin')
    ...
    if len(lmList) != 0:
        ...
        #selection mode: when two finger up
            ...
            if y1 < 128:
                ...
                if length < 40 and 0 < x1 < 50 and page > 0:
                    print('page-1=', page)
                ...
                elif length < 40 and 1220 < x1 < 1280 and page < page_max-1:
                    print('page+1=', page)
                    fname = 'image' + str(page) + '.png'
...
# TODO 試試看能不能跟影片一樣用鍵盤按'Q'停下來，才可以再停下來的時候把JPG再轉存回去PDF
# pdf = FPDF()
# for image in back_ground:
#     pdf.add_page()
#     pdf.image(image)#,x,y,w,h)      不知道這樣行不行
...
pdf_array[0].save(r'abc.pdf',save_all=True, append_images=pdf_array[1:])
# TODO 第一章多一張，黃色變藍色
